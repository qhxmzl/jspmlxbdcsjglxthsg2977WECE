...
public class Info {
	...
	//	判断用户是否已登陆,如果已登陆则通过session值来读取他所有字段值存放在hash表中
	...
	//	显示一个下拉框,三个参数,分别是该下拉框的名称,从哪个表来的数据,哪个表中的字段.
	public static String getselect(String name,String tablename,String zdname)
	{
		String select = "<select name=\""+name+"\" id=\""+name+"\" class='form-control2'>";
	...
	//	该函数与上述函数相似,多了一个"请选择"的选项,且它选择了某个值后页面会重新跳转一次,以用于加载新数据.
	public static String getselectsl(String name,String tablename,String zdname)
	{
		String select = "<select name=\""+name+"\" id=\""+name+"\" onchange='gow();' class='form-control2'>";
