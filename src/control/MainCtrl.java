...
public class MainCtrl extends HttpServlet {
	//  MainCtrl是继承自javax.http.HttpServlet的一个类
	//	在HttpServlet的service(HttpServletRequest,HttpServletResponse)方法会去判断当前请求是GET还是POST。如果是GET请求
	//	那么会去调用本类的doGet()方法，如果是POST请求会去调用doPost()方法
	...
	{
		try {
			//属于转发，也是服务器跳转，相当于方法调用，在执行当前文件的过程中转向执行目标文件
	...
	{
		try {
			response.sendRedirect(url);//地址重定向(页面跳转)
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();//e是此对象异常名称,在命令行打印异常信息在程序中出错的位置及原因
		}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("login"))//后台验证管理员登陆
		{
			String username = request.getParameter("username");//接收参数用户名
			String password = request.getParameter("pwd1"); //接收参数密码
			String utype = request.getParameter("cx");//接收参数权限
			String pagerandom = request.getParameter("pagerandom")==null?"":request.getParameter("pagerandom");//接收参数用户填写的验证码
			String random = (String)request.getSession().getAttribute("random"); //接收参数系统自动生成的验证码
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null) //判断是否提交且验证码正确
			{
				request.setAttribute("random", "");//如果不是,则验证码清空,跳回首页
				go("/index.jsp", request, response);
			}
			else{   //如果是
				String sql1="";
				if (utype.equals("管理员")) //如果权限是注册管理员
				{
					sql1 = "select * from allusers where username='"+username+"' and pwd='"+password+"'"; //查询数据库用户名与密码匹配的的数据是否存在
				}
				if(utype.equals("用户"))  //如果权限是注册用户
				{
					sql1 = "select * from yonghuxinxi where zhanghao='"+username+"' and mima='"+password+"' "; //查询数据库用户名与密码匹配的的数据是否存在
				}



				List<HashMap> userlist1 = dao.select(sql1); //执行查询
				if(userlist1.size()==1)//如果有查到
				{
					request.getSession(). setAttribute("username", username);
					if (utype.equals("管理员")){
						request.getSession(). setAttribute("cx",  userlist1.get(0).get("cx"));//给session赋值
					}
					else
					{
						request.getSession(). setAttribute("cx", utype);//给session赋值
					}
					gor("index.jsp", request, response);//跳转到指定页面
				}else{
					request.setAttribute("error", "");//否则给出错误提示
					go("/index.jsp", request, response);
				}
		...
		if(ac.equals("adminlogin"))//以下是管理员的登陆判断,与上相似
		{
			String username = request.getParameter("username"); //接收参数用户名
			String password = request.getParameter("pwd");//接收参数密码
			String utype = request.getParameter("cx");//接收参数权限
			String pagerandom = request.getParameter("pagerandom")==null?"":request.getParameter("pagerandom"); //接收参数用户填写的验证码
			String random = (String)request.getSession().getAttribute("random");//接收参数系统自动生成的验证码
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null) //判断是否提交且验证码正确
			{
				request.setAttribute("random", "");//如果不是,则验证码清空,跳转页面
				...
				if(userlist1.size()==1)//如果有查到
				{
					...
					gor("main.jsp", request, response);//跳转到指定页面
				}else{
					request.setAttribute("error", "");//否则给出错误提示
					go("/login.jsp", request, response);
				}
		...
		//修改密码
		if(ac.equals("uppass"))
		//以下是修改密码段,其原理与上述相似:先读取相似参数,进行查询匹配,如果有查询成功,则用update语句修改它密码,否则给出相应提示
		{
		...
		//修改密码
		if(ac.equals("adminuppass")) //以下是修改管理员密码,与上述相似
		{
			String olduserpass = request.getParameter("ymm");
			String userpass = request.getParameter("xmm1");
			String copyuserpass = request.getParameter("xmm2");
		...
		if(ac.equals("uploaddoc"))
		{
			try {
				...
				if(FileUpload.isMultipartContent(requestContext)){
					...
					factory.setRepository(new File(request.getRealPath("/upfile/")+"/"));; //文件缓存路径
					ServletFileUpload upload = new ServletFileUpload(factory); //创建处理工具
					upload.setSizeMax(100*1024*1024); //服务器端可以接收的最大文件大小，-1表示无上限
